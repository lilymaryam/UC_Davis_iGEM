...
def generate_promoter_mo(dnafile, jasparfile, p, n, o, freq,i,condenseddata): 
...
def get_memedata(promoter_file, meme_info, j_info, distance_scores, motif_info,jpwm,p,n,m,o):
	result = []
	fp = 0
	fn = []
	for i in range(len(meme_info)):
		...
		for j in range(len(motif_info)):
			if motif == motif_info[j][0]:
				...
				break	
		fpos,posdis,fl,overlap,overlap_p = motiflib.pos_accuracy(m_pos, j_pos,meme_wid,\
		len(jpwm))
		#print('fpos',fpos,'posdis',posdis,'fl',fl,'overlap',overlap_p)
		#print('seq','motif','nsites','mpos','m strand','meme wid','jpos','j strand', 'j wid', 'pval', 'score','p score','eval', 'f pos', 'f neg', 'posdis', 'fl', 'overlap', 'overlap_p', 'p len', 'numseq', 'model','order')
		result.append((promoter_file, seq, motif,nsites,m_pos, m_strand,meme_wid,j_pos,j_strand,len(jpwm),p_val,score,p_score,meme_eval,fpos,0,posdis,fl,overlap,overlap_p,p,n,m,o))
		#fp +=fpos
		fn.append((motif,seq))
		#result.append((seq,motif,nsites,m_strand,j_strand,overlap,overlap_p,p_val,meme_eval,\
		#score,p_score,posdis, fpos, 0,fl,p,n,m,o))
	#print(fn)
	print('get meme data','result',result)
	print('get meme data','fn',fn)
	return result,fn


#need to fix false negs everything 
def find_falsenegs(fn,j_info,p,n,m,o,promoter_file):
	#print('j_info',j_info)
	false_negs = []
	for i in range(len(j_info)):
		if j_info[i][0] not in fn:
			if j_info[i][1] != '':#j_info[i][1][0]
					false_negs.append(( promoter_file,j_info[i][0],'','','',j_info[i][1][0],'',\
					'','','',99,0,1,1,p,n,m,o))
	return false_negs	
...
def find_condensedstats(result,fn,motif_info,j_info,promoter_file,bits,p_bits,p,n,m,o):
	#print('result',result)
	condensed_stats = []
	...
	for i in range(len(motif_info)):
		...
		for j in range(len(result)):
			...
			if result[j][2] == motif_info[i][0]:
				#print('yessssss')
				#print(result[j][1], motif_info[i][0],result[j][16],result[j][13])
		...
		condensed_stats.append((promoter_file,motif_info[i][0],bits,p_bits,evalue,score,score_p, nsites,fl_ct/nsites,(nsites-fl_ct)/nsites,fp_ct/nsites,p,n,m,o))
	#print('file','motif','info','p_info','evalue','score','p score','nsites','failrate','suces rate','false p rate')
...
def present_info(promoter_file,results, bits,p_bits,scores,fn,\
	...
	false_neg = find_falsenegs(fn, j_info,p,n,m,o,promoter_file)
	if condenseddata:
		present = (find_condensedstats(results,fn,motif_info,j_info,promoter_file,bits,p_bits,p,n,m,o))
...
def get_variables_bg(memepath,jasparfile,p,n,freq,background,m,o,nummotifs,jpwm,i,condenseddata):
	promoter_file = generate_promoter_bg(jasparfile,p,n,freq,background,i,condenseddata)
	j_info = motiflib.read_testmotif(promoter_file)
	motifs, meme_info, motif_info = run_meme(memepath,promoter_file,m,o,nummotifs,background)
	scores = performance_bg(jpwm,motifs,background)
	results, fn = get_memedata(promoter_file, meme_info,j_info,scores,motif_info,\
	jpwm,p,n,m,o)
	false_negs = find_falsenegs(fn,j_info,p,n,m,o,promoter_file)
	return results, false_negs, promoter_file
	

def get_sequenceinfo_bg(memepath,jasparfile,p,n,freq,background,m,o,nummotifs,jpwm,numiterations,condenseddata):
	seqinfo = []
	#for i in range(numiterations):
	results, false_negs, promoter_file = get_variables_bg(memepath,jasparfile,p,n,freq,background,m,o,nummotifs,jpwm,numiterations,condenseddata)
...
def get_sequenceinfo_mo(memepath,dnafile,jasparfile,p,n,freq,m,o,nummotifs,jpwm,bothstrands):
	seqinfo = []
	promoter_file = generate_promoter_mo(dnafile,jasparfile,p,n,o,freq)
	j_info = motiflib.read_testmotif(promoter_file)
	motifs, meme_info, motif_info = run_meme(memepath,promoter_file,m,o,nummotifs,bothstrands)
...
def get_motifinfo_bg(memepath,jasparfile,p,n,freq,background,m,o,nummotifs,jpwm,\
	numiterations,bothstrands,bits,p_bits,condenseddata):
	result = []
	final = []
	for i in range(numiterations):
		promoter_file = generate_promoter_bg(jasparfile,p,n,freq,background,numiterations,condenseddata)
		j_info = motiflib.read_testmotif(promoter_file)
		motifs, meme_info, motif_info = run_meme(memepath,promoter_file,m,o,nummotifs,bothstrands)
		scores = performance_bg(jpwm,motifs,background)
		results, fn = get_memedata(promoter_file,meme_info,j_info,scores,\
		motif_info,jpwm,p,n,m,o)
		present = present_info(promoter_file,results,bits,p_bits,scores,fn,j_info,motif_info,p,n,m,o,condenseddata)
	...
	if numiterations > 1:
		for i in range(len(motif_info)):
			...
			#print('avg_fn',avg_fn)
			#'file','motif','info','p_info','evalue','score','p score','nsites','failrate','suces rate','false p rate
...
def get_motifinfo_mo(memepath,dnafile,jasparfile,p,n,freq,m,o,nummotifs,jpwm,numiterations,bothstrands,condenseddata,bits,p_bits):
	result = []
	final = []
	for i in range(numiterations):
		promoter_file = generate_promoter_mo(dnafile,jasparfile,p,n,o,freq,numiterations,condenseddata)
		j_info = motiflib.read_testmotif(promoter_file)
		motifs, meme_info, motif_info = run_meme(memepath,promoter_file,m,o,nummotifs,bothstrands)
		scores = performance_mo(jpwm,motifs)
		results, fn = get_memedata(promoter_file,meme_info,j_info,scores,\
...
if __name__ == '__main__':
	...
	fl = generate_promoter_bg(jasparfile, p, n, freq, background,i,condenseddata)
